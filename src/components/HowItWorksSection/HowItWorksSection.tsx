...
const HowItWorksSection = () => {
  ...
  const checklistItems = [
    "CNPJ ativo (incluindo MEI, ME, LTDA e outras categorias), sem exigência mínima de tempo para contratação.",
  ...
  return (
    <div className="text-gray-600 leading-relaxed">
      {/* Descrição */}
      <p className="text-gray-700 leading-relaxed mb-8 text-xl text-justify">
      ...
      <ul className="mt-6 space-y-6">
        {checklistItems.map((item, index) => (
          <li key={index} className="flex items-start">
            ...
            <span className="text-gray-700 leading-relaxed text-xl">{item}</span>
          </li>
        ))}
      </ul>
    </div>
